...
architecture Behavioral of PS2_ArrowKeys is
...
begin
    ...
    begin  
        if clr = '1' then 
            ...
            ps2c_filter(7) <= ps2c;                   --shift register to shift in PS2 clock 
            ps2c_filter(6 downto 0) <= ps2c_filter(7 downto 1); 
            ps2d_filter(7) <= ps2d;                   --shift register to shift in PS2 data 
        ...
        if rising_edge (clk) then           --rising edge of system clock     
            if(ps2c_int = '0') then                 --low PS2 clock, PS/2 is active 
                ...
                case ps2_word(8 DOWNTO 1) is
                    when X"1C" => Up <= '1';  -- A-KEY
                    when X"15" => Down <= '1'; -- Q-KEY
                    when others =>  -- Do nothing for other keys
                end case;
            end if;       
        end if; 
    end process;
