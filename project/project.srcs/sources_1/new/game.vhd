...
component top_PS2_Controllers is
    ...
    signal ps2_ready : std_logic; -- Een signaal om de gereedheid van de PS/2-controller aan te geven
...
begin
	...
	sevenSegDisplay : SevSeg_4digit
	 ...
	 PS2: top_PS2_Controllers
        port map (
            clk => clk,
            clr => reset,  -- Hier wordt reset als clr gebruikt, je moet mogelijk een signaal toevoegen om de reset te verbinden met clr
            ps2c => ps2c,
            ps2d => ps2d,
            Up_Arrow => upKey,   -- Verbind de signalen met de juiste signalen in je game
            Down_Arrow => downKey,
            Up_Down => openSignal,  -- Vervang openSignal door het daadwerkelijke signaal dat je hier wilt toewijzen
            Down_Down => openSignal  -- Vervang openSignal door het daadwerkelijke signaal dat je hier wilt toewijzen
        );

   process (clk, reset, ps2_ready, key_data)
begin
    if reset = '1' then
        -- Initialize the PS/2 controller
        key_ready <= '0';
        -- Andere initialisatiecode indien nodig
    elsif rising_edge(clk) then
        -- PS/2-controllerlogica om toetsgegevens te ontvangen
        -- Update key_data en key_ready op basis van de ontvangen gegevens
        key_ready <= ps2_ready; -- Aangepast op basis van de echte signaalnaam in top_PS2_Controllers
    end if;
end process;

-- Setting the motion up or down
process(clk, reset, btnU_DB, btnD_DB, key_data, key_ready, upKey, downKey)
begin
    if rising_edge(clk) then
        ...
        if reset = '1' then -- Resetting the game
            ...
            -- Direction based on FPGA buttons
            if btnU_DB = '1' or upKey = '1' then
                moveUp <= '1';
                moveDown <= '0';
            elsif btnD_DB = '1' or downKey = '1' then
                moveUp <= '0';
                moveDown <= '1';
            else
                ...
                if key_ready = '1' then
                    case key_data is
                        when "00110001" =>
                            moveUp <= '1';     -- Set moveUp based on a specific key
                            moveDown <= '0';
                        when "01011010" =>
                            moveUp <= '0';
                            moveDown <= '1';     -- Set moveDown based on a specific key
                        when others =>
                            moveUp <= '0';
                            moveDown <= '0';
                    end case;
    ...
    process(clk)
    begin
        -- Scoreteller verhogen bij elke klokpuls
        if (rising_edge(clk)) then
            score_cnt <= score_cnt + 1;
            
            -- Resetten van scoreteller en score bij resetsignaal
            if (reset = '1') then
                score_cnt <= 0;
                score <= 0;
            
            -- Score verhogen na een bepaald aantal pulsjes en als het spel niet voorbij is
    ...
    -- Weergave van score op 7-segment display
    process(score)
